...
def process_single_row(row, current_split_name, row_index):
    ...
    # Extract ground truth (align with preprocess_search_r1_dataset.py schema)
    reward_model_data = row.get("reward_model")
    if isinstance(reward_model_data, dict) and "ground_truth" in reward_model_data:
        gt = reward_model_data.get("ground_truth") or {}
        if isinstance(gt, dict):
            gt["target"] = list(gt.get("target") or [])
            reward_model_data["ground_truth"] = gt
        ground_truth = reward_model_data.get("ground_truth")
    else:
        # HotpotQA validation parquet commonly provides a single `answer` string.
        answer = row.get("answer", None)
        if answer is not None:
            targets = [answer]
        else:
            targets = list(row.get("golden_answers", []) or [])
        reward_model_data = {"style": "rule", "ground_truth": {"target": targets}}
        ground_truth = reward_model_data["ground_truth"]

    # Process data source
    data_source_tagged = "searchR1_" + 'hotpotqa_validation'
...
def main():
    ...
    with tempfile.TemporaryDirectory() as tmp_download_dir:
        src = Path(args.hf_repo_id)

        # Local file path: treat as a single split (default: test)
        if src.exists() and src.is_file():
            split = "test"
            logger.info(f"Processing local parquet as `{split}` split: {src}")
            ...
            # HF dataset (or a directory-like local repo): process train/test if present
            for split in ["train", "test"]:
                ...
                try:
                    ...
                    if src.exists() and src.is_dir() and (src / parquet_filename).exists():
                        df_raw = pd.read_parquet(src / parquet_filename)
                    else:
                    ...
                    df_processed.to_parquet(output_file_path, index=False)
                    logger.info(f"Saved {len(df_processed)} processed rows to {output_file_path}")
                    print(f"Saved {len(df_processed)} processed rows to {output_file_path}", flush=True)
                    processed_files.append(output_file_path)
